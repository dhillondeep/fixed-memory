 ...
 *
 * @section Description
 * Allocator is a memory management library where allocations are always of fixed size and
...
public:
     ...
     */
    explicit Allocator(size_t blockSize, size_t poolSize=0);
     ...
     */
    Allocator(size_t blockSize, void *pPool, size_t poolSize, Type type);
...
private:
     ...
     */
    explicit Allocator(size_t blockSize, size_t poolSize, Type allocationType, void *pPool);

    /*!
     * Helper function defined to find max of two size arguments
     *
     * @param a size number 1
     * @param b size number 2
